...
let img = $(".header__logo, .header__icon"); // Наша картинка
let company = $('.company').offset().top;
let logoH = img.position().top;

$(window).scroll(function() {
	let winScroll = $(this).scrollTop(); // На сколько проскроллили сраницу (число)

	if (winScroll > company - logoH) { // Если проскроллили меньше 1000px от начала страницы
